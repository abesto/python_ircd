...
class TestJoinCommand(unittest.TestCase):
    ...
    def test_without_topic(self):
        self.join_success(channel_topic=None, response_builder=self.joined_without_topic_response)

    def test_no_such_channel(self):
        self.mock_channel.is_valid_name.return_value = False
        self.assertListEqual(
            self.run_command(),
            [ERR_NOSUCHCHANNEL(self.channel.name, self.cmd.actor)]
        )
    ...
    def joined_without_topic_response(self):
        return [
            M(self.mock_actorcollection(self.users), 'JOIN', str(self.channel), prefix=self.joining_user),
