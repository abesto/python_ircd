...
params = (((0, 14) * (space + middle)) + Optional(space + Suppress(":") + trailing)) ^ (
...
def build_message():
    global message
    message = (
        Group(Optional(Suppress(Literal(":")) + prefix + space))
        + Group(command)
...
def wildcard(str):
    ...
    if not parsed:
        return parsed
    return (
        StringStart() + Regex("".join([wild_to_match(x) for x in parsed])) + StringEnd()
    )
